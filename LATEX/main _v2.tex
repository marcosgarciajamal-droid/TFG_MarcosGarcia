...
\end{center}
    ...
    \noindent Així mateix, faig constar que he utilitzat eines d'intel·ligència artificial generativa com a suport durant el desenvolupament tècnic i la redacció del document. L'ús d'aquestes eines s'ha aplicat principalment en les següents parts del treball: suport a la redacció i estructuració d'alguns apartats del document de manera general així com la generació d'algunes imatges i recursos gràfics (ChatGPT) i suport a la generació de parts de fragments de codi en C\# (ChatGPT, Google Gemini, Claude Code). En tot moment he mantingut l'autoria intel·lectual, el criteri tècnic i el desenvolupament íntegre de la totalitat del projecte.
...
El sistema desenvolupat incorpora diferents components habituals en entorns industrials automatitzats, com cintes transportadores, sensors virtuals, transelevadors automàtics (AS/RS), sistemes d'il·luminació d'estat i mecanismes de detecció i classificació de peces. També s'han implementat funcionalitats de simulació de fallades, temporitzadors industrials i una lògica de control basada en GRAFCET.

\bigskip

\textbf{Paraules clau:} Rapid SCADA, Unity, MQTT, automatització industrial, simulació 3D, planta virtual, supervisió industrial, GRAFCET, AS/RS, sensors virtuals.
...
\textbf{Palabras clave:} Rapid SCADA, Unity, MQTT, automatización industrial, simulación 3D, planta virtual, supervisión industrial, GRAFCET, AS/RS, sensores virtuales.
...
\bigskip
\textbf{Keywords:} Rapid SCADA, Unity, MQTT, industrial automation, 3D simulation, virtual plant, industrial supervision, GRAFCET, AS/RS, virtual sensors.
\newpage
...
Dins del projecte, Unity actua com una representació virtual de la planta industrial, simulant el comportament físic dels diferents actuadors, sensors i elements mecànics. Aquesta simulació dona l'opció de validar visualment el correcte funcionament del sistema SCADA sense necessitat de disposar d'una instal·lació industrial real.
...
Com a base principal del projecte s'han emprat les plataformes Unity i Rapid SCADA tal i com s'ha explicat en les seccions  \ref{sec:rapidscada_research} i \ref{sec:unity_research}.
...
El fet de ser un programari \textit{open source} garanteix transparència i seguretat, ja que el codi font està disponible públicament. A més, es distribueix sota la llicència Apache 2.0, la qual permet la modificació i creació de solucions derivades adaptades a necessitats específiques. Rapid SCADA s'utilitza habitualment en àmbits com:
\begin{itemize}
...
Rapid SCADA incorpora un conjunt ampli de funcionalitats que el fan adequat per a entorns industrials exigents \cite{rapidscada_intro}. Entre les més destacades es troben:
...
Els principals components del sistema són \textit{Webstation}, \textit{Server}, \textit{Communicator} i \textit{Agent} (tots explicats més endavant). Aquesta arquitectura pot implementar tant sistemes simples com arquitectures distribuïdes, on diferents serveis poden executar-se en màquines separades per millorar el rendiment i la redundància.
...
Rapid SCADA està format per un conjunt d'aplicacions que treballen conjuntament per garantir la supervisió, control i gestió de dades en temps real, les quals es comuniquen entre elles mitjançant protocols de xarxa (TCP), constituint una arquitectura distribuïda i escalable \cite{rapidscada_architecture}.
...
Facilita la transferència de configuracions i l'accés als registres del sistema, permetent treballar tant en local com de forma remota mitjançant connexió TCP \cite{rapidscada_apps}. També incorpora eines que faciliten el desenvolupament com poden ser assistents (\textit{wizards}) per crear dispositius i canals, importar i exportar configuracions de sistemes SCADA, la clonació de canal i diferents funcions de cerca, filtratge i ordenació dintre de les diferents taules del sistema. 

\subsection{Configuració amb ScadaAdmin}

La configuració del sistema SCADA s'ha realitzat mitjançant l'eina \textit{ScadaAdmin}, que actua com a entorn principal de desenvolupament \cite{rapidscada_config_basics}. Aquesta permet crear i desplegar projectes que defineixen completament el sistema mitjançant fitxers, principalment en format XML, on cada projecte de Rapid SCADA es divideix en diferents blocs principals:
...
Les \textbf{taules primàries} defineixen el funcionament principal del sistema. Són les més importants i s'han de configurar amb cura. Aquestes taules són:
...
Aquestes taules estan interrelacionades, és a dir, un dispositiu es troba associat a una línia de comunicació, i un canal està associat a un dispositiu.

\subsubsection*{Secondary Tables}


Les \textbf{taules secundàries} complementen la configuració principal del sistema amb informació auxiliar i paràmetres predefinits, ja que serveixen definir diversos aspectes principals com els tipus de dades disponibles, formats de visualització, unitats de mesura, tipus de dispositius, sistemes d'arxiu històric o scripts de càlcul. Encara que hi existieix l'opció d'ampliar aquestes taules de forma manual, generalment ja es troben suficientment definides previàment per estalviar aquest procès en cas de que no sigui estrictament necessari. 
...
Les vistes es mostren a través de la \textbf{Webstation}, on observerm que el procés per visualitzar-les correctames es basa en configura adequadament l'arxiu de la vista dintre de la interfície de \textit{ScadaAdmin}, registrar el nom de la vista a la taula de \textbf{Primary Tables} de \textit{Views}, carregar la nova configuració del projecte i accedir a la vista des del navegador web segon els permisos que hi hagi configurats. A més a més, en aquesta taula es poden configurar alguns paràmetres com objecte associat, ordre o configuració específica (Figura \ref{fig:rapidscada_views}).
...
És a dir, la \textit{Communication Line} estableix el tipus de connexió (TCP, sèrie, MQTT, etc.), mentre que el \textit{Device} defineix com es comunica realment amb aquell equip. En alguns casos, com amb \textit{OPC UA}, la configuració pot dependre gairebé exclusivament del dispositiu.
...
Els canals \cite{rapidscada_channels} són les unitats bàsiques de dades dins del SCADA (equivalents a variables o tags) i cada canal es troba enllaçat a un dispositiu i a una variable concreta del dispostiu al qual està associat(sensor, estat, actuador, ...). Les propietats més rellevants són:
...
Els usuaris es defineixen a la taula \textit{Users} i ,en el moment de la seva creació, s'ha d'especificar quin rol compleixen dintre del sistema. Però, de forma base ja hi ha alguns usuaris i rols creats per defecte, com ara:
\begin{itemize}
...
Independentment d'això, la part rellevant és que existeix la possibilitat de crear els teus propis rols amb permisos configurables, simplificant la gestió en sistemes complexos.

\subsubsection*{Permisos i control d'accés}

Els permisos es defineixen a la taula \textit{Object rights}, indicant què pot fer cada rol sobre cada objecte. Aquest sistema permet assignar exactament quin tipus d'accins poden realitzar els diferents rols, com ara visualitzar dades, tindre accés a unes pantalles en concret o tindre la capacitat d'enviar comandes a través d'uns objectes/canals específics. 
...
A part de l'emmagatzematge intern, Rapid SCADA permet integrar bases de dades externes com \textbf{PostgreSQL} o \textbf{InfluxDB} mitjançant mòduls i plugins específics \cite{rapidscada_db_export}. Aquesta funcionalitat facilita:
...
Aquest plugin permet crear interfícies gràfiques més avançades i flexibles que els fitxers \texttt{.sch} bàsics de RapidSCADA, on les principals diferències dels esquemes tradicionals són:
\begin{itemize}
...
Unity dona l'opció de desenvolupar aplicacions tant en 2D com en 3D dins d'un mateix entorn de treball. Per un costat, en entorns 2D, proporciona eines específiques com el sistema de \textit{Sprites}, física 2D independent i càmeres ortogràfiques, facilitant el desenvolupament d'aplicacions basades en gràfics plans.
...
Com a factor interessant, consta d'un sistema de renderització en temps real basat en la GPU, on es poden visualitzar escenes de forma eficient. Ofereix diferents tipus de \textit{Render Pipelines} \cite{UnityRenderPipelines}, entre els quals destaquen:
\begin{itemize}
...
A la vista \textit{Scene} \cite{UnityScenes, UnityEditorInterface} s'observa l'espai on es construeix el món virtual. Permet visualitzar i manipular els objectes en un entorn 2D o 3D, modificar la seva posició, rotació i escala, així com organitzar-los dins de l'escena. Aquesta vista és \textbf{interactiva} i no representa el resultat final del joc, sinó una eina de disseny.

A la vista \textit{Game} \cite{UnityEditorInterface} es mostra la representació final del joc tal com la veuria l'usuari. Aquesta vista depèn de les càmeres de l'escena i permet executar el projecte en temps real mitjançant el botó de \textit{Play}.

En l'\textit{Inspector} \cite{UnityEditorInterface} es pot visualitzar i modificar totes les propietats dels objectes seleccionats. Quan es selecciona un objecte, l'Inspector mostra tots els seus components (transformacions, materials, scripts, etc.) i permet editar-los (figura \ref{fig:menu_editor_inspector}).

A la finestra \textit{Hierarchy} \cite{UnityEditorInterface} es mostra tots els objectes (\textit{GameObjects}) presents en l'escena actual, organitzats en forma d'arbre jeràrquic (establiment de relacions pare-fill).
...
Unity té la capacitat d'assignar \textit{Tags} i \textit{Layers} als objectes per facilitar la seva identificació i gestió dintre de la edició de la simulació, fet que ajuda a tindre una estructura més concreta \cite{UnityTags2023}.

\begin{itemize}
    \item \textbf{Tags}: identificació d'objectes segons les necessitats del desenvolupador (per exemple, "Player" o "Box"). Ajuda a separar les funcions dels \textit{GameObjects} a l'hora d'interactuar amb els scripts segons el tag que tinguin. 
...
El \textit{Package Manager} \cite{UnityPackageManager} és una eina integrada en Unity que afegeix i gestiona funcionalitats addicionals mitjançant paquets, els quals inclouen recursos, scripts i eines que amplien les capacitats del motor. Aquesta eina facilita la instal·lació i actualització de components dins del projecte des d'una única interfície (figura \ref{fig:package_manager}). En aquest projecte s'han utilitzat alguns paquets rellevants \cite{UnityPackagesDocs}:
...
En Unity, els \textit{materials} defineixen l'aspecte visual dels objectes de l'escena, incloent propietats com el color, la brillantor o la interacció amb la llum \cite{UnityMaterialsIntro}. Cada material està associat a un \textbf{Shader}, responsable de determinar com es representa visualment la superfície \cite{UnityShaders} i els materials s'assignen als objectes mitjançant el component \textit{Mesh Renderer} que serveix per visualitzar el \textit{GameObject}, i poden ser compartits entre diversos elements de l'escena.%\cite{UnityMaterialsReference}. 

En aquest projecte s'han utilitzat principalment textures PBR (\textit{Physically Based Rendering}) importades des de repositoris externs per aconseguir una representació més realista dels diferents components industrials. Les textures s'importen directament a la carpeta \textit{Assets} i posteriorment s'assignen als canals corresponents del material \cite{UnityImportTextures} al crear un de nou. Entre els mapes utilitzats per crear un nou material destaquen: 
\begin{itemize} 
    \item \textbf{Albedo}: defineix el color base del material. 
    \item \textbf{Normal Map}: simula relleu i detall superficial. 
    \item \textbf{Metallic Map}: controla el comportament metàl·lic davant la llum. 
    \item \textbf{Height Map}: aporta sensació de profunditat. 
    \item \textbf{Ambient Occlusion}: genera ombrejats suaus en zones menys il·luminades. 
\end{itemize} 

Com a factor addicional, existeix la possibilitat  permet modificar propietats addicionals dels materials, com el color, l'\textit{offset} o diferents paràmetres d'il·luminació, facilitant així la reutilització i personalització visual dels objectes dins de la simulació \cite{UnityTextureProperties}. Finalment, l'ús del model PBR permet mantenir una aparença coherent sota diferents condicions d'il·luminació de l'escena \cite{UnityMaterialsIntro}. 
...
El \textit{Rigidbody} \cite{UnityRigidbody} permet aplicar física als objectes, com forces, gravetat i col·lisions, sent essencial en simulacions com el moviment de peces on es busca que hi hagi interacció de peces i que tingui un comportament relativament realista. El \textit{Rigidbody} disposa de diverses propietats que controlen el seu comportament:
...
\textbf{Diferència entre \textit{isKinematic} i \textit{useGravity}}: Dins del component \textit{Rigidbody}, dues propietats especialment rellevants són \textit{isKinematic} i \textit{useGravity}, ja que determinen com l'objecte interactua amb el sistema de física de Unity.
...
\begin{itemize}
    ...
    \item \textbf{isKinematic}:  Quan està activada, el \textit{Rigidbody} deixa de ser controlat pel motor físic. Això significa que no respon a forces ni a col·lisions físiques i que el moviment s'ha de controlar manualment mitjançant scripts \cite{UnityRigidbody,UnityColliders}.
\end{itemize}
...
Els \textit{Colliders} \cite{UnityColliders} defineixen la forma física d'un objecte per a la detecció de col·lisions. No són visibles, però són essencials per a la interacció entre objectes, el que provoca que l'ús correcte del tipus de collider és important per equilibrar precisió i rendiment del sistema. Unity ofereix diferents tipus de colliders segons la geometria:
...
Poden funcionar en mode \textit{Trigger}, permetent detectar objectes sense col·lisió física, fet útil per simular sensors industrials. Aquest mode s'utilitza mitjançant esdeveniments com \texttt{OnTriggerEnter()}, \texttt{OnTriggerExit()} o \texttt{OnTriggerStay()}. Aquest tipus de detecció és molt comú a la part de útil en sistemes industrials per simular sensors de presència, dit això, obviament en aquest sistema s'jan utilitzat \textit{colliders} amb aquest mode per simular aquests tipus de sensors.
...
El component \textit{Renderer} \cite{UnityRenderer} és responsable de la representació visual dels objectes dins de l'escena. Aquest component permet mostrar un objecte utilitzant un model 3D i un material associat i, en el cas de un objecte no tingués un \textit{Renderer} assignat, l'objecte no seria visible dins de la escena. Aquest component utilitza materials que es troben dintre del prjecte per definir l'aparença visual de l'objecte, els quals inclouen shaders i textures que determinen com interactua amb la llum i quins tipus de relleu tenen.
...
\begin{itemize}
    ...
    \item \textbf{FixedUpdate}:  S'executa a intervals de temps fixos i s'utilitza principalment per a càlculs físics. Normalment, s'ha d'utilitzar per treballar amb \textit{Rigidbody}, mentre que \texttt{Update} s'utilitza per la lògica general \cite{UnityUpdate}.
\end{itemize}
...
Unity permet accedir als components \cite{UnityComponents} d'un objecte mitjançant la funció \texttt{GetComponent<>()}. Aquesta funció retorna una referència a un component específic associat al mateix \textit{GameObject}. Un exemple d'ús d'aquesta funció és \texttt{Rigidbody rb = GetComponent<Rigidbody>();}
...
La funció \texttt{Vector3.MoveTowards()} \cite{UnityMoveTowards} és un exemple de canvi de posició, on s'encarrega de desplaçar un objecte de forma progressiva cap a una posició objectiu sense sobrepassar-la. Aquest mètode calcula internament una interpolació lineal controlada, garantint un moviment suau i estable. Dintre de Unity, sol ser molt utilitzat a l'hora de generar elevadors o de objectes que han d'arribar a una posició específica.

\subsection{Fonaments avançats de programació}

\subsubsection{Creació d'objectes en temps d'execució}

Unity dona la opció de generar objectes en temps d'execució mitjançant la funció \texttt{Instantiate()} \cite{UnityInstantiate}.  Aquesta funció crea una còpia d'un objecte \textit{prefab} dins de l'escena, mecanisme el qual és bastant utilitzat en diferents situacions per generar peces de manera automàtica mitjançant el sistema de \textit{spawner}, tal i com succeeix en el el sistema dissenyat d'aquest projecte.
...
El terme \textit{Quaternion} prové de les matemàtiques i fa referència a una extensió dels nombres complexos a quatre dimensions. Un quaternion està format per quatre components (x, y, z, w), però aquests no representen angles directes, sinó una representació matemàtica de la rotació \cite{UnityQuaternionEuler2019}.

\subsubsection*{Problema del Gimbal Lock} %% PENSAR EN GIMBAL LOCK, QUITO O NO
...
Per aquest motiu, Unity utilitza internament quaternions, ja que no pateixen el problema de \textit{Gimbal Lock}, permeten interpolacions suaus i són més estables numèricament.
...
Els objectes amb component \textit{Rigidbody} \cite{UnityRigidbody} poden ser controlats mitjançant velocitats enlloc de posicions. En aquesta context, la propietat \texttt{linearVelocity} defineix la velocitat lineal de l'objecte en l'espai. En la situació del nostre sistema, es calcula una velocitat en una direcció concreta, només es modifiquen els eixos horitzontals i es manté la component vertical per no interferir amb la gravetat, aixó aconseguim que el \textit{GameObject} es mogui de forma realista sobre els diferent components, mantenint la interacció física amb el sistema.
...
Aquest tipus de sintaxi s'anomena \textit{expression-bodied member} \cite{MicrosoftExpressionBodied} i serveix per definir propietats de forma més compacta i llegible.
...
Aquests elements utilitzen un component especial anomenat \textit{Rect Transform}, que ajuda posicionar-los en funció de la pantalla i adaptar-los a diferents resolucions. 
...
\begin{itemize}
    ...
    \item \textbf{UNE-EN 1090-1}: regula els requisits relacionats amb el disseny i fabricació d'estructures metàl·liques utilitzades en entorns industrials.
\end{itemize}
...
La simulació d'una planta industrial dintre de Unity realitzada e aquest projecte representa un sistema automatitzat de classificació i emmagatzematge de peces. La lògica del procés s'executa íntegrament en Unity, mentre que Rapid SCADA actua com a sistema de supervisió, monitoratge i enviament de comandes d'operació. Aquesta separació és intencionada i respon a criteris d'arquitectura industrial moderna:
...
Quan una estanteria arriba a la seva capacitat màxima (nou peces en aquest cas), el sistema inicia automàticament el procés d'evacuació de les peces emmagatzemades cap a la zona de sortida de la planta, gràcies a uns rodets que es troben incorporats en cada estantería (que serveixen tant per recol·locar les peces internament com per evacuar-les).
...
\textbf{Sensors} $\rightarrow$ \textbf{plant/sensors/Si},  on \textit{i} correspon al número del sensor definit a la Taula \ref{tab:tab_sensors}.
...
\textbf{Actuadors} $\rightarrow$ \textbf{plant/actuators/Mi}, on \textit{i} correspon al número del motor o actuador definit a la Taula \ref{tab:tab_actuadors}.
...
Aquests topics representen les ordres enviades des de Rapid SCADA cap a Unity per controlar el comportament general de la planta virtual (taula \ref{tab:mqtt_general_cmd}).
...
\textbf{Control manual d'actuadors} $\rightarrow$ Els actuadors controlats manualment segueixen la nomenclatura \textbf{plant/cmd/manual/Mi} on \textit{i} correspon al motor o actuador associat.
...
Per delimitar la zona de col·lisió amb els objectes (\textit{collider}), es disposa de dos colliders dintre del mateix objecte amb dues funcions per cadascuna: 
...
Per evitar errors amb múltiples peces simultànies, s'utilitza un comptador intern enlloc de la funció \texttt{OnTriggerStay()}, reduint també el cost computacional. Quan una peça amb etiqueta \textit{Box} entra dins del sensor, aquest activa la variable interna del sensor i encén el LED associat, i en el moment en el que no hi ha cap pecça dintre del \textit{collider} (comptador intern a 0), apaga la variable del LED associat.
...
El control es realitza mitjançant variables com \textit{isOk}, \textit{emergency} i \textit{manual}, i en el moment en el que alguna d'aquestes variables s'activa, s'habilita o es deshabilita el \textit{GameObject} a través de la funció \texttt{SetActive()}, simulant l'encesa real (secció \ref{sec:scripts_unity}). Per generar l'efecte intermitent de la llum vermella, s'utilitza un temporitzador basat en \texttt{Time.deltaTime}, que permet alternar l'estat de la llum de forma periòdica \cite{UnityTime}.
...
Cada peça també incorpora un sistema de col·lisió \cite{UnityColliders} adaptat a la seva geometria per tal de garantir una simulació física precisa, on la peça de \textbf{Tipus A i B} consta d'un \textit{Box Collider} (eficient i adequat per formes simples), però la peça \textbf{Tipus C} té un \textit{Mesh Collider} convex per adaptar-se a la seva geometria triangular, treta del paquet \textit{ProBuilder}, amb la forma \textit{Prism}.
...
El GRAFCET d'evacuació (G80) gestiona el buidatge automàtic de les estanteries quan aquestes arriben a la seva capacitat màxima.  Aquest programa coordina els moviments del segon transelevador i dels sistemes de rodets per transferir les peces des de les estanteries fins a la cinta d'evacuació. El procés inclou:
...
Alguns exemples de GRAFCETs complets d'actuadors es poden consultar a l'Annex \ref{sec:grafcets_g100}.
...
Aquest programa supervisa la \textbf{posició} del selector de mode, els \textbf{botons} d'arrencada i aturada, les \textbf{condicions} de seguretat i \textbf{l'estat del sistema d'emergència}.
...
Abans de permetre qualsevol transició entre aquests modes, el sistema verifica que els actuadors es troben en una posició segura i que es compleixen les condicions inicials de funcionament. A més a més, per sortir d'un mode com per entrar a l'altre, és necessari que l'operador polsi el botó de \textit{Start} o el botó de \textit{Stop} respectivament per verificar el canvi de mode, i aquesta transició no es realitzarà fins que el sistema es trobi en condicions segures per realitzar el canvi de mode. La representació completa del GRAFCET G500 es pot consultar a l'Annex \ref{sec:grafcets_g500}.
...
La representació completa del GRAFCET G600 es mostra a l'Annex \ref{sec:grafcets_g500}, figura \ref{fig:annex_g600}.

\subsubsection{Detecció d'avaries}

La detecció d'avaries s'ha implementat mitjançant un sistema de supervisió temporal basat en un \textit{Dual TWEEN}, mecanisme que permet detectar situacions en què una determinada transició del procés no es completa dins del temps esperat.
...
Aquest comportament permet simular i validar la resposta del sistema davant condicions anòmales de funcionament. El GRAFCET complet de detecció d'avaries es pot consultar a l'Annex \ref{sec:grafcets_g500}, figura \ref{fig:annex_gdt0}.

\subsection{Implementació del control en Unity}
Per a la simulació del sistema industrial s'ha desenvolupat un script principal anomenat \textit{PlantController}, el qual actua com a nucli de control de tota la planta dins de Unity. Aquest script conté totes les variables públiques associades als elements del sistema, incloent sensors, actuadors, comptadors i sistemes auxiliars.
...
El GRAFCET d'emergència té la màxima prioritat dins del sistema i pot interrompre qualsevol altre procés en execució. L'emergència es pot activar polsant botó \texttt{B\_Emergency} o amb la detecció d'avaria en el sistema de classificació (\texttt{ETAPA\_DT0 = 21}). Quan s'activa l'emergència, es controla de forma adequada tots els esdeveniments comentats en la secció \ref{sec:grafc_emerg} (desactivació funcionament automàtic, s'encén la llum d'emergència, es reinicien tots els GRAFCETS de procès a través de la funció \texttt{ResetGrafcetsProcess()} i es força el sistema al mode manual fent \texttt{ETAPA\_500 = 503})
...
Com podreu observar, aquesta funció implementada per mi de \texttt{Rising\_Edge} s'ha aplicat tant per introduir una peça en l'etapa que toca com modificar el valors dels 6 comptadors que hi ha dintre del sistema en les etapes corresponent. Com únicament és necessari fer-ho quan s'activa l'etapa (\textbf{acció impulsional}), aquesta funció és perfecta per aquestes situacions.

És fonamental \textbf{reiniciar} l'estat del diccionari per cada clau quan es torna a l'etapa inicial del GRAFCET, en cas contrari, el sistema podria no detectar correctament futurs flancs.
...
La integració del protocol MQTT a Unity s'ha realitzat mitjançant la llibreria \textit{M2MqttUnity}, la qual serveix per establir connexions amb un broker MQTT i gestionar tant la publicació com la subscripció a diferents \textit{topics} \cite{mqtt_unity_guide}. A partir d'aquesta estructura base proporcionada per la llibreria, s'ha desenvolupat la lògica específica del projecte, incloent la gestió dels missatges rebuts i enviats, així com la sincronització amb l'estat intern del sistema industrial virtual. Tant la configuració del broker com la implementació general de la comunicació MQTT s'han basat en la documentació oficial de Mosquitto \cite{mosquitto_download}.
...
La connexió amb el broker MQTT es realitza dins del mètode \texttt{Start()}, que s'executa una sola vegada en iniciar la simulació \cite{mqtt_unity_guide}:
...
Aquesta codificacó permet representar el mode del sistema de forma simple, d'igual manera que l'estat d'emergència, facilitant la seva interpretació des del sistema SCADA seguint les etapes que hi hagi actives en el procès d'automatització.
...
Un cop definida la línia de comunicació, el següent pas és crear un dispositiu associat a aquesta línia. Dins d'aquest dispositiu, cal configurar el \textbf{Driver}: \texttt{DrvMqttClient}, el qual és l'encarregat de gestionar la comunicació MQTT dins de RapidSCADA.
...
\textbf{Subscripcions (\textit{Subscriptions})}:  Especifiquen els \textit{topics} als quals RapidSCADA es subscriurà per rebre dades des de Unity. Per a cada subscripció es defineix:
\begin{itemize}
    \item El \textbf{topic MQTT}
    \item Un \textbf{identificador o tag}
    \item El tipus d'accés (normalment només lectura)
    \item Opcionalment, un codi en JavaScript per processar les dades rebudes ( no s'ha utilitzat processament addicional, ja que cada variable es publica en un \textit{topic} independent)
\end{itemize}
...
\textbf{Comandes (\textit{Commands})}: Defineixen els \textit{topics} als quals RapidSCADA enviarà informació. Aquests \textit{topics} corresponen a les ordres que controlen el comportament de la planta en Unity, com per exemple ordres manuals o canvis de mode de funcionament.
...
Una vegada configurat el dispositiu, és necessari crear els canals (\textit{channels}) \cite{rapidscada_channels} que representaran cadascuna de les variables dins del sistema SCADA i els quals actuan com a punt d'accés a les dades. Per a cada canal es defineix la línia de comunicació associada, el dispositiu MQTT, l'objecte (habitualment \textit{Enterprise} en cas de no haver creat cap objecte específic) i el valor o índex del canal corresponent.

\subsubsection*{Configuració avançada dels canals}

Finalment, tots els canals creats es poden visualitzar i configurar des de la base de dades de configuració, concretament a la taula \textit{Channels} dins de \textit{Primary Table}. En aquesta taula es poden ajustar diversos paràmetres com poden ser el tipus d'accés (lectura o escriptura), el tipus de dada (per exemple: \textit{On-Off}, \textit{Yes-No}) o el nombre de decimals. 
...
\label{sec:hmi}
Continuant amb els conceptes de comunicació mitjançant MQTT i la integració amb Rapid SCADA descrits en l'apartat anterior, en aquesta secció es detalla com es dissenyen les pantalles HMI (\textit{Human-Machine Interface}) que tenen la funció visualitzar i interactuar amb les variables del sistema.
...
Inicialment, es va crear una vista de tipus \texttt{.tbl} anomenada \textit{System Overview}, amb l'objectiu de visualitzar totes les variables del sistema a través dels canals configurats. Aquesta vista permetia observar en temps real els valors dels tòpics MQTT, però presentava una limitació important: la manca d'una representació visual intuïtiva. Per aquest motiu, es van desenvolupar \textbf{tres pantalles HMI} orientades a millorar la visualització de cada variable i el control del sistema.

Pel que fa a la navegació entre pantalles, aquesta es realitza exclusivament mitjançant el menú predefinit de Rapid SCADA (figura \ref{fig:menu_hmi}), ja que l'eina no permet implementar una estructura de navegació interna personalitzada dins de les pròpies vistes, limitant així la interfícide de l'usuari.
...
Les dades generades a Unity (com sensors de presència, posició o comptadors) són publicades mitjançant MQTT i rebudes a Rapid SCADA a través dels canals d'entrada. Aleshores, aquesta pantalla visualitza en temps real l'estat d'aquestes variables, facilitant la comprensió del comportament del sistema.
...
Per a la seva implementació s'ha utilitzat el tipus \textit{Mimic Diagram} \cite{RapidScadaMimicVideo}, el qual a diferència del format \texttt{.sch}, aquest permet enviar comandes directament als canals mitjançant la funcionalitat \textit{Send Command} amb un valor concret, facilitant la interacció amb el sistema. Aquesta pantalla inclou:
...
Aquest mètode permet simular un selector físic dins de l'entorn HMI (figura \ref{fig:selector_mode}). La imatge del selector està vinculada a un \textit{input channel}, de manera que mostra en tot moment l'últim valor assignat a aquest element, i en cas de pulsar qualsevol de les tres zones, el valor del selector canviarà a la vegada que s'envia per MQTT. 
...
Per solucionar aquesta limitació  trobada en el element \textit{Button} de RapidSCADA \cite{RapidScadaVideos}, es va implementar una tècnica basada en l'ús de \textbf{toggles invisibles} superposats als elements de control. Cada botó tenia per sobre un toggle que es va associar a un canal configurat com a \textit{input} i \textit{output}, fet que permetia saber l'estat actual del botó en tot moment.

El funcionament consisteix en un botó visual amb una imatge dinàmica vinculada al canal del botó, sobre el qual es col·loca un toggle invisible que, en ser activat, envia el valor contrari a l'estat actual. D'aquesta manera, s'aconsegueix un comportament bidireccional coherent amb la representació gràfica del botó.
...
Cal destacar també que la correcta configuració de \textit{Chart Pro} és necessària per visualitzar múltiples canals. En cas contrari, només es mostrarà un únic canal. De la mateixa manera passa amb el \textit{ExtraSchemeEditor}, que és necessari per veure el gràfic en aquesta mateixa pantalla, sinó la única manera de visualitzar els gràfics és a través de la taula.
...
S'han creat diferents usuaris associats als rols definits, cadascun amb les seves credencials d'accés. Per tal de simplificar la gestió, els noms dels usuaris coincideixen amb els rols corresponents. La configuració de les credencials es realitza des de l'eina d'administració de Rapid SCADA, on es poden definir les contrasenyes i assignar els rols de manera individual. S'han creat els usuaris \textit{User}, \textit{Supervisor} i \textit{Operator} (amb el seu mateix rol) i en cas de que ara mateix es volgui accedir des de qualsevol usuari, la contrassenya és \textit{1234}. \footnote{També existeix per defecte l'usuari \textit{admin} amb contrassenya \textit{scada}, que serveix per revisar amb tots els permisos disponibles qualsevol pantalla.}
...
En segon lloc, es planteja la migració del sistema a un entorn amb \textbf{accés remot mitjançant IP pública}, permetent l'accés remot a la Webstation de Rapid SCADA. Aquest fet facilitaria les demostracions i l'escalabilitat del sistema desenvolupat en entorns distribuïts.   A més, permetría utilitzar un broker MQTT públic i controlat personalment, possibilitant la connexió automàtica al sistema sense requerir cap configuració en el dispositiu de l'usuari.  En cas d'aplicar aquesta millora, seria necessari implementar mesures de seguretat adequades, com ara l'ús de connexions segures (TLS) i autenticació per garantir la protecció de les dades i el control d'accés al sistema.
...
Pel que fa a la visualització i explotació de dades, seria convenient aprofundir en l'ús de \textbf{plugins i mòduls de RapidSCADA}, així com integrar \textbf{bases de dades externes} (com PostgreSQL o InfluxDB). Aquesta integració és especialment rellevant, ja que el sistema intern de Rapid SCADA es basa en fitxers \texttt{.dat} i no permet la realització de consultes SQL directes, limitant l'anàlisi avançada de la informació. Respecte a la part dels plguins de RapidSCADA, l'óptim seria tindre una clau permanent d'aquests plugins que permetés utilitzar-ho amb tota la llibertat possible, sense necessitat de demanar una \textit{Trial Key} cada dia \cite{store_scada}.
...
%\subsubsection*{\textbf{C.0} Llegendes i convencions}
%\addcontentsline{toc}{subsubsection}{\textbf{C.0} Llegendes i convencions}
%
...
\begin{subfigure}{0.48\textwidth}
    ...
    \caption{Vista lateral esquerra}
\end{subfigure}
