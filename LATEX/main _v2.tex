...
En concret, aquestes eines s'han utilitzat com a suport per a la generació d'esborranys de text, ajuda en la programació de scripts relacionats amb Unity, revisió de fragments de documentació tècnica, assistència en la presentació formal del treball i per la generacions d'imatges que es poden observar en les diferents pantalles de la secció \ref{sec:hmi}
...
Els canals són les unitats bàsiques de dades dins del SCADA (equivalents a variables o tags) i cada canal es troba enllaçat a un dispositiu i a una variable concreta del dispostiu al qual està associat(sensor, estat, actuador, ...). Les propietats més rellevants són:
...
Malgrat això, la part rellevant és que existeix la possibilitat de crear els teus propis rols amb permisos configurables, simplificant la gestió en sistemes complexos.
...
El \textbf{Mimic Editor} és un plugin que permet crear interfícies gràfiques més avançades i flexibles que els fitxers \texttt{.sch}. A diferència dels esquemes tradicionals, aquest editor permet:
\begin{itemize}
...
Com a factor interessant, consta d'un sistema de renderització en temps real basat en la GPU, on es poden visualitzar escenes de forma eficient \cite{GPUWikipedia}. Ofereix diferents \textit{Render Pipelines} \cite{UnityRenderPipelines}, entre els quals destaquen:
\begin{itemize}
...
A la finestra \textit{Hierarchy} \cite{UnityHierarchy} es mostra tots els objectes (\textit{GameObjects}) presents en l'escena actual, organitzats en forma d'arbre jeràrquic (establiment de relacions pare-fill).
...
\begin{itemize}
    \item \textbf{Tags}: identificació d'objectes segons les necessitats del desenvolupador (per exemple, "Player" o "Box").
...
En Unity, els \textit{materials} defineixen l'aspecte visual dels objectes de l'escena, incloent propietats com el color, la brillantor o la interacció amb la llum \cite{UnityMaterialsIntro}. Cada material està associat a un \textbf{Shader}, responsable de determinar com es representa visualment la superfície \cite{UnityShaders}. 

Els materials s'assignen als objectes mitjançant el component \textit{Mesh Renderer}, i poden ser compartits entre diversos elements de l'escena \cite{UnityMaterialsReference}. 

En aquest projecte s'han utilitzat principalment textures PBR (\textit{Physically Based Rendering}) importades des de repositoris externs per aconseguir una representació més realista dels diferents components industrials. Les textures s'importen directament a la carpeta \textit{Assets} i posteriorment s'assignen als canals corresponents del material \cite{UnityImportTextures} al crear un de noy. Entre els mapes utilitzats per crear un nou material destaquen: 
\begin{itemize} 
...
Els AS/RS server a fer un ús eficient de l'espai vertical del magatzem, incrementant la densitat d'emmagatzematge i millorant la gestió de l'inventari. A més, contribueixen a la reducció de costos laborals, a la millora de la seguretat i a una major precisió en les operacions logístiques.
...
El disseny del sistema també depèn de les estanteries utilitzades \cite{rack_estanterias_norma}. La norma \textbf{UNE-EN 15620} defineix toleràncies i precisió en sistemes d'emmagatzematge. Segons aquesta norma, les estanteries es poden classifacar segons diferents tipus de classes que serveixen per determinal el nivell d'automatització i precisió del sistema:
...
La peça és transferida al primer transelevador, que la posiciona segons el tipus detectat al lloc predeterminat. Consta d'un sensor de presència (\textbf{S3}) per detectar si la peça ha arribat correctament a la plataforma d'elevació. Aquest transelevador consta de 4 nivell verticals i 4 nivells horitzontals, on en cada posició trobem un sensor que ens ajuda a conèixer en tot moment la ubicació del transelevador, en els dos eixos.
...
\textbf{Estat del sistema} $\rightarrow$ Les variables globals d'estat informen del mode actual de funcionament del sistema. El topic \textbf{plant/status/actuation mode} indica el mode de control actual, seguint la convenció definida a continuació:
...
\textbf{Control manual d'actuadors} $\rightarrow$ Els actuadors controlats manualment segueixen la nomenclatura \textbf{plant/cmd/Mi} on \textit{i} correspon al motor o actuador associat.
...
L'objectiu principal d'aquesta implementació  era reproduir el comportament d'una petita planta industrial automatitzada intentant que tot tingui un semblant realista ja sigui en l'apartat visual, en la funcionalitat del sistema i en la simplicitat de simulació. Si es vol observar unes imatges de com ha quedat la planta industrial, les trobareu en la secció del Annex \ref{sec:images_planta}
...
Cada peça també incorpora un sistema de col·lisió d'un sistema de col·lisió \cite{UnityColliders} adaptat a la seva geometria per tal de garantir una simulació física precisa, on la peça de \textbf{Tipus A i B} consta d'un \textit{Box Collider} (eficient i adequat per formes simples), però la peça \textbf{Tipus C} té un \textit{Mesh Collider} convex per adaptar-se a la seva geometria triangular, treta del paquet \textit{ProBuilder}, amb la forma \textit{Prism}.
...
En concret, el sistema utilitza una operació de tipus mòdul (per exemple, $C_A mod 3==0$)per determinar la ubicació final de cada peça dins del cicle de classificació, de manera que les peces es van assignant seqüencialment a les diferents posicions disponibles de forma controlada i segura. 
...
Pel que fa als transelevadors, tal i com s'ha explicat previàment, quan el transelevador arriba a la posició inicial, la comanda s'ha de forçar a un valor neutre (\texttt{-1}), evitant així que el sistema torni a executar automàticament el mateix moviment.
...
Un diccionari permet emmagatzemar parelles \textit{clau-valor}, on la clau és el \textit{topic MQTT} i el valor és l'últim valor enviat.
