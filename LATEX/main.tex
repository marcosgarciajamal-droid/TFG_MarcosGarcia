...
\begin{titlepage}
 ...
 %   \vspace{0.5cm}
%    \href{https://escriny.epsem.upc.edu/svn/tfg-marcosgarcia}
    \vfill
...
\label{sec:rollers}
La plataforma del transelevador disposa d'un sistema de rodets que permet introduir o extreure les caixes. El seu funcionament és similar al d'una cinta transportadora, amb la diferència que pot operar en ambdues direccions (codi \ref{lst:rollerconveyor}).  L'script detecta objectes amb \textit{Rigidbody} \cite{UnityRigidbody} dins d'un collider \textit{trigger} i aplica una velocitat en la direcció definida. Aquesta direcció es pot invertir mitjançant el paràmetre \textit{forward}.

Tanmateix, per reforçar l'efecte visual, s'ha implementat un script que desplaça la textura del material dels rodets (codi \ref{lst:rollervisual}). Aquest només s'activa quan el sistema està en funcionament, mantenint coherència entre el moviment visual i físic.
...
El sensor òptic permet identificar el tipus de peça mitjançant un script associat a cada objecte (codi \ref{lst:pieshapedata}).  Quan una peça entra dins del sensor, es llegeix aquesta informació i, després d'un retard controlat mitjançant un temporitzador, es valida la detecció (codi \ref{lst:opticalsensor}).  Aquest retard simula el temps de resposta d'un sensor real i permet al sistema prendre decisions en funció del tipus de peça. A l'apartat \ref{sec:timers} s'explica com s'ha implementat aquest retard. 
...
El GRAFCET general (G0, Figura \ref{fig:grafcet_general}) defineix el flux principal del procés automàtic. Aquest consta de 19 etapes o estats del sistema amb les seves respectives tansicions, les quals, quan són validades, permeten el canvi d'estat. En aquest cas, no es controlen directament actuadors, sinó que es coordinen els diferents subprocessos del sistema. Les etapes i les transicions implicades són:

\begin{itemize}[leftmargin=*, nosep]
\item \textbf{E0-E1:} Estat inicial. Comprovació de condicions inicials (CI).
...
\item \textbf{E15-E17:} Avalua l'estat d'altres GRAFCET per poder retornar a l'etapa inicial, concretament els estats E28, E48 i E68.
...
\subsubsection{GRAFCET de classificació}
...
\item \textbf{E22:} Decisió segons el comptador de peces de l'estanteria A (C\_A mod 3) passar a una de les tres possibles estanteries (E23, E24 o E25).:
  \begin{itemize}
  \item Si C\_A mod 3 = 0, la col·loca a l'estanteria A, estat representat per E23.
  \item Si C\_A mod 3 = 1, la col·loca a l'estanteria B, estat representat per E24.
  \item Si C\_A mod 3 = 2, la col·loca a l'estanteria C, estat representat per E25.
  \end{itemize}
...
\begin{itemize}
\item Selector: 0 (automàtic), 1 (manual), -1 (neutre)
...
En aquest estat:
\begin{itemize}
\item Si el selector està en posició automàtica (0), es prem \textbf{B\_Start} i el sistema no es troba en mode emergència (\textbf{E600} actiu), es passa a \textbf{E501}.
\item Si el selector està en posició manual (1),es prem \textbf{B\_Start} i el sistema no es troba en mode emergència (\textbf{E600} actiu), es passa a \textbf{E503}.
\end{itemize}
...
\item Es surt d'aquest estat si es prem el botó d'aturada (\textbf{B\_Stop}), independentment de la posició del selector. En aquest cas es passa a \textbf{E502}.
\end{itemize}
...
Aleshores el sistema passa a \textbf{E602}, on es verifica si el sistema compleix les condicions inicials i que el mode manual no es troba actiu. Un cop complert, es retorna a \textbf{E600}. Això evita reinicis perillosos i garanteix que el sistema es reprengui únicament des d'una situació controlada.
...
El controlador es basa en una arquitectura centralitzada on tots els GRAFCETs s'executen de manera seqüencial dins del mètode \texttt{Update()}, el qual s'executa a cada frame del motor de Unity. A nivell d'estrucrura, cada GRAFCET està implementat com una funció independent i tots els GRAFCETs són cridats seqüencialment a cada iteració del cicle
...
Cada valor representa una etapa concreta del GRAFCET, i els canvis d'estat es realitzen mitjançant estructures \texttt{switch-case}. Exemple d'implementació: 
...
Aquest GRAFCET centralitza tant la selecció del mode de funcionament com l'activació general del sistema. Els elements de control associats són \texttt{B\_Start}, \texttt{B\_Stop} (normalment tancat) i el\texttt{Selector}de mode (\texttt{0} automàtic, \texttt{1} manual, \texttt{-1} neutre). Per passar d'un estat a un altre és la mateixa explicació que s'ha explicat en la secció \ref{sec:section_selector}
...
\newpage
\section{Condicions per a la seva execució}
...
\subsection*{\textbf{A} Scripts generals del sistema}
\addcontentsline{toc}{subsection}{\textbf{A} Scripts generals del sistema}

\subsection*{Classe Timer}

\begin{lstlisting}[style=modern, caption={Classe Timer}, label={lst:timer}]
using UnityEngine;
public class Timer
{
    public float preset;
...
\subsection*{Controlador de càmera}

\begin{lstlisting}[style=modern, caption={Controlador de càmera lliure}, label={lst:freecamera}]
using UnityEngine;
...
\subsection*{Sistema de llums}

\begin{lstlisting}[style=modern, caption={Sistema de llums d'estat}, label={lst:lightsystem}]
using UnityEngine;
...
\end{lstlisting}
\subsection*{Generador de peces}

\begin{lstlisting}[style=modern, caption={Sistema de generació de peces}, label={lst:spawner}]
using UnityEngine;
...
\subsection*{Eliminació de peces}

\begin{lstlisting}[style=modern, caption={Sistema d'eliminació de peces}, label={lst:destroypiece}]
using UnityEngine;
...
\subsection*{\textbf{B} Scripts relacionats amb actuadors}
\addcontentsline{toc}{subsection}{\textbf{B} Scripts relacionats amb actuadors}

\subsection*{Control de cintes transportadores}

\begin{lstlisting}[style=modern, caption={Controlador de cinta transportadora}, label={lst:motorbelt}]
using UnityEngine;
...
\subsection*{Sistema de rodets}

\begin{lstlisting}[style=modern, caption={Controlador de rodets}, label={lst:rollerconveyor}]
using UnityEngine;
...
\subsection*{Visualització dels rodets}

\begin{lstlisting}[style=modern, caption={Visualització de rodets}, label={lst:rollervisual}]
using UnityEngine;
...
\begin{lstlisting}[style=modern, caption={Moviment horitzontal del transelevador}, label={lst:asrshorizontal}]
using UnityEngine;
...
\subsection*{Moviment vertical del transelevador}

\begin{lstlisting}[style=modern, caption={Moviment vertical del transelevador}, label={lst:asrsvertical}]
using UnityEngine;
...
\section*{\textbf{C} Scripts relacionats amb sensors}
\addcontentsline{toc}{subsection}{\textbf{C} Scripts relacionats amb sensors}

\subsection*{Definició del tipus de peça}

\begin{lstlisting}[style=modern, caption={Definició del tipus de peça}, label={lst:pieshapedata}]
using UnityEngine;
public enum PieceShape { Square, Cylinder, Triangle }
public class PieceData : MonoBehaviour { public PieceShape shape; }
\end{lstlisting}

\subsection*{Sensor òptic amb possible fallada}

\begin{lstlisting}[style=modern, caption={Sensor òptic}, label={lst:opticalsensor}]
using UnityEngine;
...
\subsection*{Visualització del sensor òptic}

\begin{lstlisting}[style=modern, caption={Visualització del sensor òptic}, label={lst:opticalsensordisplay}]
using UnityEngine;
using TMPro;
...
\subsection*{Sensor de presència}

\begin{lstlisting}[style=modern, caption={Sensor de presència}, label={lst:presencesensor}]
using UnityEngine;
...
\begin{lstlisting}[style=modern, caption={Indicador LED}, label={lst:ledindicator}]
using UnityEngine;
...
\subsection*{Visualització dels comptadors}


\begin{lstlisting}[style=modern, caption={Visualització dels comptadors}, label={lst:displaycounter}]
using UnityEngine;
using TMPro;
